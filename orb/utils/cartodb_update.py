...
def run(cartodb_account, cartodb_key, source_site):
    ...
    for c in carto_db_data['rows']:
        location = UserLocationVisualization.objects.filter(
    ...
    # add any new points
    locations = UserLocationVisualization.objects.exclude(lat=0, lng=0).values(
        'lat', 'lng', 'country_code').annotate(total_hits=Sum('hits'))
    for l in locations:
        found = False
